...
public:
    int maximumScore(int a, int b, int c) {
        priority_queue<int> t1_pri_que;
        t1_pri_que.emplace(a); t1_pri_que.emplace(b);t1_pri_que.emplace(c);

        int score = 0;

        while(t1_pri_que.size()>1) {
            int t1_val = t1_pri_que.top(); t1_pri_que.pop();
            int t2_val = t1_pri_que.top(); t1_pri_que.pop();
            ...
            if(t1_val>0) {
                t1_pri_que.emplace(t1_val);
            }

            if(t2_val>0) {
                t1_pri_que.emplace(t2_val);
            }
        }
        return score;
    }
